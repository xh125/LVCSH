  ...
  CONTAINS
    ...
    IF (ierr /= 0) CALL errore('print_gkk', 'Error allocating epc_sym', 1)
		allocate(epc_cmp(nbndfst,nbndfst,nmodes,nktotf),STAT = ierr)
    ...
    CALL mp_sum(epc_cmp, inter_pool_comm )
    ...
    IF (mpime == ionode_id) THEN
      ...
      DO ik = 1, nktotf
        ...
        WRITE(stdout, '(5x, a)') ' ibnd     jbnd     imode   enk[eV]    enk+q[eV]      omega(q)[meV]   |g|[meV]            Re[gmnvkq][meV]     Im[gmnvkq][meV]'
        WRITE(stdout, '(5x, a)') REPEAT('-', 127)
        !
        DO ibnd = 1, nbndfst
          ...
          DO jbnd = 1, nbndfst
            ...
            DO nu = 1, nmodes
              WRITE(stdout, '(3i9, 2f12.4, 1f20.10, 3e20.10)') ibndmin - 1 + ibnd, ibndmin - 1 + jbnd, &
