module cc_fssh
  ...
  subroutine get_G_CC_FSSH(nfre,nfre_sh,isurface,isurface_j,p0,p,w0,w,S_ai,g1,g)
    ...
    complex(kind=dp),intent(in) :: p0(nfre,nfre)
    ...
    S_ai = 0.0
    isurface_a = isurface
    S_aa = SUM(p0(:,isurface_a)*CONJG(p(:,isurface_a)))
    ...
    if(S_aa**2 > 0.5) then
      ...
      !type(2) or type(4)
      do ifre = 1,nfre_sh
        S_ai(ifre) = ABS(SUM(p0(:,isurface_a)*CONJG(p(:,ifre))))**2
  ...
  subroutine nonadiabatic_transition_ccfssh(lfeedback,nfre,nfre_sh,nmodes,nq,&
    ...
    complex(kind=dp),intent(in) :: P(nfre,nfre),P0(nfre,nfre)
    ...
    do ifre=1,nfre_sh
      if(ifre /= isurface_a) then
        ...
        if(flagr < sumgg) then
          ...
          S_bi = 0.0
          do jfre = 1,nfre_sh
            S_bi(jfre) = ABS(SUM(CONJG(p0(:,isurface_b))*p(:,jfre)))**2
          enddo
          !S_bi = S_bi**2
          max_Sbi =  MAXLOC(S_bi)
